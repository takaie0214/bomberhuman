...
#[no_mangle]
pub extern "C" fn toggle_to_up(b: c_int) {
    let data = &mut DATA.lock().unwrap();
    data.actions.to_up = int_to_bool(b);
}

#[no_mangle]
pub extern "C" fn toggle_to_down(b: c_int) {
    let data = &mut DATA.lock().unwrap();
    data.actions.to_down = int_to_bool(b);
}

#[no_mangle]
pub extern "C" fn toggle_to_left(b: c_int) {
    let data = &mut DATA.lock().unwrap();
    data.actions.to_left = int_to_bool(b);
}

#[no_mangle]
pub extern "C" fn toggle_to_right(b: c_int) {
    let data = &mut DATA.lock().unwrap();
    data.actions.to_right = int_to_bool(b);
}
